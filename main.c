...
Grid CreateGrid(int x, int y, int w, int h)
{
    ...
    g.width = w;
    g.height = h;
    g.cells = (Cell*)calloc(w * h, sizeof(Cell));
...
int* GenerateBaseTable(int arr_size)
{
    ...
    int* results = (int*)calloc(arr_size * totalLines, sizeof(int));
...
{
    int* results = (int*)calloc(op.size, sizeof(int));
...
Table BuildTable(TableInfo ti)
{
    ...
    t.operatorTable.gOperator = (Grid*)calloc(8, sizeof(Grid));
...
int main()
{
    ...
    TableInfo ti;
    ti.hLabels = (char*)calloc(8, sizeof(char));
    ti.hLabels[0] = 'P';
    ti.hLabels[1] = 'Q';
    ti.hLabels[2] = 'R';
    ti.hLabels[3] = 'S';
